...
export class DashboardComponent implements OnInit {
  ...
  searchTerm: string = ''; // New property to hold search input
  currentPageforCompare: number = 1;
  itemsPerPageforCompare: number = 5;
  ...
  applyFilter(filter: 'MTD' | 'QTD' | 'YTD') {
    ...
    this.activeFilterTopCards = filter; // make sure this is set for fetchTopCards()
  ...
  loadDashboardMetrics(filter: 'MTD' | 'QTD' | 'YTD') {
    ...
    // 1️⃣ API call to get overall metrics (no userIds param)
    const overallUrl = `https://uat.smartassistapp.in/api/dealer/dealer/updatedAnalysis/dashboard?type=${filter}`;

    this.http.get<any>(overallUrl, { headers }).subscribe({
      next: (response) => {
        const data = response?.data || {};
        ...
        this.dashboardMetrics = {
          ...
          netOrders: data.netOrders || 0,
          retail: data.retail || 0,
          performance: [], // we'll update this below
          allIndiaBestPerformace: data.allIndiaBestPerformace || {
            enquiriesCount: 0,
            testDrivesCount: 0,
            newOrdersCount: 0,
        ...
        // 2️⃣ If userIds selected, fetch filtered user performance for table
        if (selectedUserIds) {
          const filteredUrl = `https://uat.smartassistapp.in/api/dealer/dealer/updatedAnalysis/dashboard?type=${filter}&userIds=${selectedUserIds}`;

          this.http.get<any>(filteredUrl, { headers }).subscribe({
            next: (filteredResp) => {
              const filteredData = filteredResp?.data || {};

              // Set filtered user performance data in table
              this.selectedUsersPerformance = (
            ...
            error: (err) => {
              console.error('Filtered dashboard data fetch error', err);
              this.selectedUsersPerformance = []; // fallback empty
            },
          });
        } else {
          // If no user filter, show all users performance from first call
          this.selectedUsersPerformance = (data.performance || []).map(
            (user: any) => ({
              userId: user.user_id,
              name: user.name || '',
              enquiries: user.enquiries || 0,
